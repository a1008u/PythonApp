...
from AppRobot.roboter.models import ranking
from AppRobot.roboter.views import console
...
class Robot(object):
    ...
    def __init__(self, name=DEFAULT_ROBOT_NAME, user_name='', speak_color='green'):
        ...
        self.speak_color: str = speak_color

    def hello(self):
        """Returns words to the user that the robot speaks at the beginning."""
        while True:
            template: Template = console.get_template('hello.txt', self.speak_color)
            user_name: str = input(template.substitute({'robot_name': self.name}))

            if user_name:
                self.user_name: str = user_name.title()
                break
...
class RestaurantRobot(Robot):
    ...
    def recommend_restaurant(self):
        ...
        will_recommend_restaurants: list = [new_recommend_restaurant]
        while True:
            template: Template = console.get_template('greeting.txt', self.speak_color)
            is_yes: str = input(template.substitute({
            ...
            if is_yes.lower() == 'n' or is_yes.lower() == 'no':
                new_recommend_restaurant: str = self.ranking_model.get_most_popular(not_list=will_recommend_restaurants)
                if not new_recommend_restaurant:
                    break
    ...
    def ask_user_favorite(self):
        ...
        while True:
            template: Template = console.get_template('which_restaurant.txt', self.speak_color)
            restaurant: str = input(template.substitute({
                'robot_name': self.name,
                'user_name': self.user_name,
            }))
            if restaurant:
    ...
    def thank_you(self):
        ...
        template: Template = console.get_template('good_by.txt', self.speak_color)
